...
| A raw bug report from someone else, not yet confirmed or written up | triage first |
| Throwaway code to answer a design question, not chase a defect | prototype |
| Building a planned behaviour test-first | tdd |
| No good seam exists to lock the bug down | improve-codebase-architecture: this skill hands off there itself |
...
`diagnosing-bugs` is a reach-for-it-anytime standalone. You drop into it when something is broken and drop out when the fix and its regression test are in; it holds no state and needs no prior setup. ask-sk routes "Something's broken" here.
