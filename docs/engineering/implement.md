...
You invoke this by typing `/implement` yourself: the agent won't reach for it on its own. It ships with `disable-model-invocation: true`, so no other skill can call it either. Wherever ask-sk or to-tickets says "then `/implement` per ticket", that is an instruction to you, not something the agent will do unprompted.
...
If the tickets came from to-tickets, the tracker they live on was configured by setup-sk-skills. `code-review` reads the same configuration to find the originating spec at close-out.
