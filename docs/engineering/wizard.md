...
Don't reach for it to *decide* what to build; for that, grill-with-docs and to-spec are the tools.
...
`wizard` is a reach-for-it-anytime standalone, sitting at the line where automation stops and a human has to click. Its nearest neighbour is setup-sk-skills, because both exist to get a repo into a working state: that one configures this skill set, while `wizard` generates a setup path for everything else. It also pairs with implement: when a build lands a feature that needs credentials or a manual cutover, a wizard is how the human half gets done. When you're unsure which skill fits the moment, ask-sk routes you.
