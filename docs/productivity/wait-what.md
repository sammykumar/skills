...
Use it the second you notice you're skimming. The agent has drifted into jargon it invented, stacked five acronyms, or explained a decision whose premise you never saw. It fixes the conversation you're already in. To stop the jargon arriving at all, use grill-with-docs, which builds the shared language upfront.
...
You can use `wait-what` at any point, in any conversation, inside any other skill. It repairs one message after the fact. The real cure is a shared language agreed upfront, and that is grill-with-docs: a grilling session that runs domain-modeling as it goes, so the words you both use land in your `CONTEXT.md`. If you're unsure which skill fits the moment, ask-sk routes you.
