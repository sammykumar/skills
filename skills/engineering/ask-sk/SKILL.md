...
- **`/figma-arch-diagram`** draws an architecture, tech-stack, networking, or data-flow diagram on a **FigJam board** from the published "Icon Lib - Editable" card library. **Nothing is placed by hand**: the diagram is authored as a spec, a tested engine solves every coordinate, and Figma only ever receives already-solved numbers. Reach for it when a system needs to be *shown* to someone, most often after `/grill-with-docs` or `/diagnosing-bugs` has settled what the system actually is. Not for Mermaid, and not for the Figma MCP server's own `generate_diagram`.
- **`/openscad-lsp`** is the one entry here whose subject is a piece of **plugin configuration** rather than a process: the plugin wires an OpenSCAD language server into Claude Code over `.scad` files, and this skill is the **capability boundary** for it. Cross-file go-to-definition works and is the reason it exists; find references does not exist at all, rename only ever rewrites the current file, and diagnostics are parse errors rather than anything semantic. Reach for it whenever OpenSCAD source is in play, so a symbol the server never resolved does not come back as a confident answer.
